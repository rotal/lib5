 ...
 * Pure translations don't need baking - just matrix updates.
 */
...
/**
 * Check if transform has rotation (b or c components non-zero).
 * Rotation can cause content clipping at image edges.
 */
...
/**
 * Check if a baking check is needed for this transform.
 * Returns true if transform has rotation (which can cause edge clipping).
 * Scale alone doesn't change bbox shape so doesn't need baking check.
 */
...
/**
 * Check if all edge pixels of an image match the default color.
 * Used to determine if rotation would clip visible content.
 */
export function hasEmptyEdges(image: FloatImage, defaultColor: Color): boolean {
  const { data, width, height } = image;
  const tol = 1 / 255; // Tolerance for floating point comparison

  const colorMatches = (idx: number): boolean => {
    return (
      Math.abs(data[idx] - defaultColor.r) < tol &&
      Math.abs(data[idx + 1] - defaultColor.g) < tol &&
      Math.abs(data[idx + 2] - defaultColor.b) < tol &&
      Math.abs(data[idx + 3] - defaultColor.a) < tol
    );
  };

  // Check top row
  for (let x = 0; x < width; x++) {
    if (!colorMatches(x * 4)) return false;
  }

  // Check bottom row
  const bottomRowStart = (height - 1) * width * 4;
  for (let x = 0; x < width; x++) {
    if (!colorMatches(bottomRowStart + x * 4)) return false;
  }

  // Check left column (excluding corners already checked)
  for (let y = 1; y < height - 1; y++) {
    if (!colorMatches(y * width * 4)) return false;
  }

  // Check right column (excluding corners already checked)
  for (let y = 1; y < height - 1; y++) {
    if (!colorMatches((y * width + width - 1) * 4)) return false;
  }
...
/**
 * Determine if a transform should be baked (pixels resampled) before passing to a node.
 * Smart baking only resamples when necessary to prevent content clipping.
 *
 * Bake transforms ONLY when ALL conditions are true:
 * 1. Transform includes rotation (not pure translation)
 * 2. Edge pixels differ from default color (content at edges might clip)
 *
 * Never bake for:
 * - Identity transforms
 * - Pure translations (just update transform matrix)
 * - Rotations when edges are "empty" (match default color)
 */
export function shouldBakeTransform(image: FloatImage, defaultColor: Color): boolean {
  ...
  if (isPureTranslation(t)) return false;
  if (!transformRequiresBakingCheck(t)) return false;

  // Has rotation - check if edges are empty (match default color)
 ...
 *
 * @param image The source image with transform to bake
 * @param defaultColor Color to use when sampling outside source bounds (default: transparent black)
 */
export function applyTransformToImage(
  ...
  // Default color channel values for sampling outside bounds
  const defaultChannels = [defaultColor.r, defaultColor.g, defaultColor.b, defaultColor.a];

  // Get pixel from source with bilinear interpolation
  const sampleSource = (sx: number, sy: number, channel: number): number => {
    if (sx < 0 || sx >= srcW || sy < 0 || sy >= srcH) {
      return defaultChannels[channel]; // Use default color outside source bounds
    }
  ...
  // Attach a translation transform to position the baked image in world space
  // Pixel (0,0) of output corresponds to world position (minX, minY)
  return {
    data: dst,
    width: dstW,
    height: dstH,
    transform: translateTransform(minX, minY),
  };
}
