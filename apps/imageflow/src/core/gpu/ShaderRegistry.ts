...
const MASK_BLEND_FRAG = `#version 300 es
precision highp float;

// Blend between original and processed image using a mask.
// mask red channel: 1.0 = use processed, 0.0 = use original.
...
void main() {
  ...
  vec4 proc = texture(u_processed, v_texCoord);
  float m = texture(u_mask, v_texCoord).r;
  fragColor = mix(orig, proc, m);
}
`;
...
export class ShaderRegistry {
  ...
  private dynamicShaders: Map<string, string> = new Map();
  ...
  /**
   * Register a dynamic shader with the given name and source.
   * If a program with this name already exists, it will be invalidated and recompiled on next use.
   */
  registerShader(name: string, source: string): void {
    // If source hasn't changed, skip
    if (this.dynamicShaders.get(name) === source) {
      return;
    }

    this.dynamicShaders.set(name, source);

    // Invalidate cached program if it exists
  ...
  /**
   * Check if a shader with the given name exists (static or dynamic)
   */
  hasShader(name: string): boolean {
    return name in ShaderRegistry.FRAGMENT_SHADERS || this.dynamicShaders.has(name);
  }
  ...
  getProgram(name: string): ShaderProgramInfo | null {
    ...
    // Get shader source (static first, then dynamic)
    const fragmentSource = ShaderRegistry.FRAGMENT_SHADERS[name] ?? this.dynamicShaders.get(name);
    if (!fragmentSource) {
