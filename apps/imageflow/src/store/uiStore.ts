...
const PREVIEW_STORAGE_KEY = 'lib5-preview';
...
function savePreviewState(state: Pick<UiState, 'previewSlots' | 'previewBackgroundActive' | 'previewForegroundSlot' | 'previewSplitPosition' | 'previewSplitVertical' | 'previewSplitReversed'>) {
  localStorage.setItem(PREVIEW_STORAGE_KEY, JSON.stringify(state));
}

const restoredPreview = loadPreviewState();
...
useUiStore.subscribe(
  (state, prev) => {
    if (
      state.previewSlots !== prev.previewSlots ||
      state.previewBackgroundActive !== prev.previewBackgroundActive ||
      state.previewForegroundSlot !== prev.previewForegroundSlot ||
      state.previewSplitPosition !== prev.previewSplitPosition ||
      state.previewSplitVertical !== prev.previewSplitVertical ||
      state.previewSplitReversed !== prev.previewSplitReversed
    ) {
      savePreviewState({
        previewSlots: state.previewSlots,
        previewBackgroundActive: state.previewBackgroundActive,
        previewForegroundSlot: state.previewForegroundSlot,
        previewSplitPosition: state.previewSplitPosition,
        previewSplitVertical: state.previewSplitVertical,
        previewSplitReversed: state.previewSplitReversed,
      });
    }
  }
);
